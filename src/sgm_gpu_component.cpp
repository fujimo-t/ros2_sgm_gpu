...
SgmGpuComponent::SgmGpuComponent() : Node("sgm_gpu", options)
{
  ...
  uint8 sgm_p1_ = declare_parameter("p1", 6);
  uint8 sgm_p2_ = declare_parameter("p2", 96);
  bool check_consistency_ = declare_parameter("check_consistency", true);
...
)
{
  ...
  RCLCPP_INFO(get_logger(), "Elapsed time: %f [ms]", elapsed_time_ms);
